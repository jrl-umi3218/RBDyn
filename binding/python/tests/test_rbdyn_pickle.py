    ...
    def test(self):
        ...
        def makeBody(bName):
            inertia = sva.RBInertiad(
                e3.Vector3d.Random().x(), e3.Vector3d.Random(), e3.Matrix3d.Random()
            )
            return rbd.Body(inertia, bName)

        body = makeBody("testBody")

        jR = rbd.Joint(rbd.Joint.Rev, e3.Vector3d.Random().normalized(), True, "jR")
        jP = rbd.Joint(rbd.Joint.Prism, e3.Vector3d.Random().normalized(), False, "jP")
        jS = rbd.Joint(rbd.Joint.Spherical, False, "jS")
        jPla = rbd.Joint(rbd.Joint.Planar, True, "jPla")
        jC = rbd.Joint(
            rbd.Joint.Cylindrical, e3.Vector3d.Random().normalized(), False, "jC"
        )
        jFree = rbd.Joint(rbd.Joint.Free, True, "jFree")
        jFix = rbd.Joint(rbd.Joint.Fixed, False, "jFix")

        mb = rbd.MultiBody(
            [makeBody("body%s" % i) for i in range(7)],
            [jR, jP, jS, jPla, jC, jFree, jFix],
            list(range(-1, 6)),
            list(range(0, 7)),
            list(range(-1, 6)),
            [sva.PTransformd(e3.Vector3d(0.0, i, 0.0)) for i in range(7)],
        )
        ...
        def bodyEq(b1, b2):
            return b1.inertia() == b2.inertia() and b1.name() == b2.name()

        def jointEq(j1, j2):
            return (
                j1.type() == j2.type()
                and j1.name() == j2.name()
                and j1.direction() == j2.direction()
                and list(j1.motionSubspace()) == list(j2.motionSubspace())
            )
        ...
        def multiBodyEq(mb1, mb2):
            ...
            mb1T = (
                list(mb1.predecessors()),
                list(mb1.successors()),
                list(mb1.parents()),
                list(mb1.transforms()),
            )
            mb2T = (
                list(mb2.predecessors()),
                list(mb2.successors()),
                list(mb2.parents()),
                list(mb2.transforms()),
            )

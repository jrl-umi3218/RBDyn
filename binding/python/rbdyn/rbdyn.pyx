...
cdef DoubleVectorVectorWrapper DoubleVectorVectorWrapperFromC(
    vector[vector[double]] & v_in, owner
):
...
cdef class Body(object):
    ...
    def __comictor__(
        self, double mass, eigen.Vector3d com, eigen.Matrix3d inertia, name
    ):
        if isinstance(name, unicode):
            name = name.encode(u"ascii")
    ...
    def __cinit__(self, *args):
        if len(args) == 0:
            ...
            len(args) == 4
            and isinstance(args[1], eigen.Vector3d)
            and isinstance(args[2], eigen.Matrix3d)
        ):
...
cdef class MultiBody(object):
    ...
    def __ctor__(
        ...
        succ,
        parent,
        sva.PTransformdVector Xt
    ):
        self.impl = new c_rbdyn.MultiBody(
            bodies.v, joints.v, pred, succ, parent, deref(Xt.v)
        )
...
cdef class MultiBodyGraph(object):
    ...
    def linkBodies(
        ...
        self.impl.linkBodies(
            b1Name, deref(tB1.impl), b2Name, deref(tB2.impl), jointName, isB1toB2
        )
    ...
    def mergeSubBodies(self, rootBodyName, jName, arg):
        if isinstance(arg, dict) and len(arg) > 0 and isinstance(arg.values()[0], list):
            self.impl.mergeSubBodies(
                rootBodyName, jName, <cppmap[string, vector[double]]>arg
            )
    ...
    def __makeMB(
        ...
        rBName,
        cppbool isFixed,
        sva.PTransformd X_0_j0 = sva.PTransformd.Identity(),
        sva.PTransformd X_b0_j0 = sva.PTransformd.Identity()
    ):
        return MultiBodyFromC(
            self.impl.makeMultiBody(
                rBName, isFixed, deref(X_0_j0.impl), deref(X_b0_j0.impl)
            )
        )
    ...
    def __makeMBWAxis(
        ...
        return MultiBodyFromC(
            self.impl.makeMultiBody(
                rBName, tp, ax.impl, deref(X_0_j0.impl), deref(X_b0_j0.impl)
            )
        )
            ...
            if isinstance(args[0], bool):
                ...
                if len(args) > 2 and isinstance(args[2], sva.PTransformd):
                    if len(args) == 4    and isinstance(args[3], sva.PTransformd):
                        return self.__makeMBWAxis(
                            rootBodyName, args[0], args[1], args[2], args[3]
                        )
                    elif len(args) == 3:
                        return self.__makeMBWAxis(
                            rootBodyName, args[0], args[1], args[2]
                        )
                elif len(args) == 2:
    ...
    def bodiesBaseTransform(
        self, rootBodyName, sva.PTransformd X_b0_j0 = sva.PTransformd.Identity()
    ):
        cdef cppmap[string, c_sva.PTransformd] res = self.impl.bodiesBaseTransform(
                rootBodyName, deref(X_b0_j0.impl)
        )
        ret = {}
        for it in res:
    ...
    def successorJoints(self, rootBody):
        cdef cppmap[string, vector[string]] res = self.impl.successorJoints(rootBody)
        return res
...
cdef MultiBodyGraph MultiBodyGraphFromC(
    const c_rbdyn.MultiBodyGraph & mbg, cppbool copy = True
):
...
cdef MultiBodyConfig MultiBodyConfigFromC(
        const c_rbdyn.MultiBodyConfig& mbc, cppbool copy = True
):
...
cdef MultiBodyConfigVector MultiBodyConfigVectorFromPtr(
    vector[c_rbdyn.MultiBodyConfig]* p
):
...
cdef class ConfigConverter(object):
    ...
    def __mbsctor__(self, MultiBody mb1, MultiBody mb2):
        self.impl = c_rbdyn_private.ConfigConverterConstructor(
            deref(mb1.impl), deref(mb2.impl)
        )
    ...
    def __cinit__(self, *args):
        if len(args) == 1 and isinstance(args[0], ConfigConverter):
            ...
            len(args) == 2
            and isinstance(args[0], MultiBody)
            and isinstance(args[1], MultiBody)
        ):
...
cdef class Jacobian(object):
    ...
    def __mbvctor__(
        self, MultiBody mb, bodyName, eigen.Vector3d point = eigen.Vector3d.Zero()
    ):
    ...
    def jacobian(self, MultiBody mb, MultiBodyConfig mbc, sva.PTransformd X_0_p = None):
        if X_0_p is None:
            return eigen.MatrixXdFromC(
                self.impl.sJacobian(deref(mb.impl), deref(mbc.impl))
            )
        else:
            return eigen.MatrixXdFromC(
                self.impl.sJacobian(deref(mb.impl), deref(mbc.impl), deref(X_0_p.impl))
            )

    def jacobianDot(self, MultiBody mb, MultiBodyConfig mbc):
        return eigen.MatrixXdFromC(
            self.impl.sJacobianDot(deref(mb.impl), deref(mbc.impl))
        )

    def bodyJacobian(self, MultiBody mb, MultiBodyConfig mbc):
        return eigen.MatrixXdFromC(
            self.impl.sBodyJacobian(deref(mb.impl), deref(mbc.impl))
        )

    def bodyJacobianDot(self, MultiBody mb, MultiBodyConfig mbc):
        return eigen.MatrixXdFromC(
            self.impl.sBodyJacobianDot(deref(mb.impl), deref(mbc.impl))
        )

    def vectorJacobian(self, MultiBody mb, MultiBodyConfig mbc, eigen.Vector3d v):
        return eigen.MatrixXdFromC(
            self.impl.sVectorJacobian(deref(mb.impl), deref(mbc.impl), v.impl)
        )

    def vectorBodyJacobian(self, MultiBody mb, MultiBodyConfig mbc, eigen.Vector3d v):
        return eigen.MatrixXdFromC(
            self.impl.sVectorBodyJacobian(deref(mb.impl), deref(mbc.impl), v.impl)
        )

    def translateJacobian(
        self,
        eigen.MatrixXd jac,
        MultiBodyConfig mbc,
    ...
    def velocity(self, MultiBody mb, MultiBodyConfig mbc, sva.PTransformd X_b_p = None):
        if X_b_p is None:
            return sva.MotionVecdFromC(
                self.impl.sVelocity(deref(mb.impl), deref(mbc.impl))
            )
        else:
            return sva.MotionVecdFromC(
                self.impl.sVelocity(deref(mb.impl), deref(mbc.impl), deref(X_b_p.impl))
            )

    def bodyVelocity(self, MultiBody mb, MultiBodyConfig mbc):
        return sva.MotionVecdFromC(
            self.impl.sBodyVelocity(deref(mb.impl), deref(mbc.impl))
        )

    def __normalAccelerationPTMV(
        self,
        MultiBody mb,
        MultiBodyConfig mbc,
        sva.PTransformd X_b_p,
        sva.MotionVecd V_b_p
    ):
        return sva.MotionVecdFromC(
            self.impl.sNormalAcceleration(
                deref(mb.impl), deref(mbc.impl), deref(X_b_p.impl), deref(V_b_p.impl)
            )
        )

    def __normalAccelerationAccPTMV(
        self,
        MultiBody mb,
        MultiBodyConfig mbc,
        normalAccB,
        ...
        return sva.MotionVecdFromC(
            self.impl.sNormalAcceleration(
                deref(mb.impl),
                deref(mbc.impl),
                sva.MotionVecdVector(normalAccB).v,
                deref(X_b_p.impl),
                deref(V_b_p.impl),
            )
        )
    ...
    def normalAcceleration(self, MultiBody mb, MultiBodyConfig mbc, *args):
        if len(args) == 0:
            return sva.MotionVecdFromC(
                self.impl.sNormalAcceleration(deref(mb.impl), deref(mbc.impl))
            )
        if len(args) == 1 and isinstance(args[0], list):
            return sva.MotionVecdFromC(
                self.impl.sNormalAcceleration(
                    deref(mb.impl), deref(mbc.impl), sva.MotionVecdVector(args[0]).v
                )
            )
        if (
            len(args) == 2
            and isinstance(args[0], sva.PTransformd)
            and isinstance(args[1], sva.MotionVecd)
        ):
            return self.__normalAccelerationPTMV(mb, mbc, args[0], args[1])
        if (
            len(args) == 3
            and isinstance(args[0], list)
            and isinstance(args[1], sva.PTransformd)
            and isinstance(args[2], sva.MotionVecd)
        ):
    ...
    def bodyNormalAcceleration(
        self, MultiBody mb, MultiBodyConfig mbc, normalAccB = None
    ):
        if normalAccB is None:
            return sva.MotionVecdFromC(
                self.impl.sBodyNormalAcceleration(deref(mb.impl), deref(mbc.impl))
            )
        else:
            return sva.MotionVecdFromC(
                self.impl.sBodyNormalAcceleration(
                    deref(mb.impl), deref(mbc.impl), sva.MotionVecdVector(normalAccB).v
                )
            )
...
def forwardAcceleration(
        MultiBody mb,
        MultiBodyConfig mbc,
        sva.MotionVecd A_0 = sva.MotionVecd(eigen.Vector6d.Zero())
):
...
def computeCoMVelocity(MultiBody mb, MultiBodyConfig mbc):
    return eigen.Vector3dFromC(
        c_rbdyn.sComputeCoMVelocity(deref(mb.impl), deref(mbc.impl))
    )


def computeCoMAcceleration(MultiBody mb, MultiBodyConfig mbc):
    return eigen.Vector3dFromC(
        c_rbdyn.sComputeCoMAcceleration(deref(mb.impl), deref(mbc.impl))
    )
...
cdef class CoMJacobian(object):
    ...
    def normalAcceleration(self, MultiBody mb, MultiBodyConfig mbc, mvecvec = None):
        if mvecvec is None:
            ...
            return eigen.Vector3dFromC(
                self.impl.sNormalAcceleration(
                    deref(mb.impl), deref(mbc.impl), sva.MotionVecdVector(mvecvec).v
                )
            )


def computeCentroidalMomentum(MultiBody mb, MultiBodyConfig mbc, eigen.Vector3d com):
    return sva.ForceVecdFromC(
        c_rbdyn.sComputeCentroidalMomentum(deref(mb.impl), deref(mbc.impl), com.impl)
    )


def computeCentroidalMomentumDot(
    MultiBody mb, MultiBodyConfig mbc, eigen.Vector3d com, eigen.Vector3d comVel
):
    return sva.ForceVecdFromC(
        c_rbdyn.sComputeCentroidalMomentumDot(
            deref(mb.impl), deref(mbc.impl), com.impl, comVel.impl
        )
    )
...
cdef class CentroidalMomentumMatrix(object):
    ...
    def computeMatrixDot(
        self, MultiBody mb,
        MultiBodyConfig mbc,
        eigen.Vector3d com,
        eigen.Vector3d comDot
    ):
        self.impl.sComputeMatrixDot(
            deref(mb.impl), deref(mbc.impl), com.impl, comDot.impl
        )

    def computeMatrixAndMatrixDot(
        self,
        MultiBody mb,
        MultiBodyConfig mbc,
        eigen.Vector3d com,
        ...
        self.impl.sComputeMatrixAndMatrixDot(
            deref(mb.impl), deref(mbc.impl), com.impl, comDot.impl
        )
    ...
    def momentum(self, MultiBody mb, MultiBodyConfig mbc, eigen.Vector3d com):
        return sva.ForceVecdFromC(
            self.impl.sMomentum(deref(mb.impl), deref(mbc.impl), com.impl)
        )

    def normalMomentumDot(
        self,
        MultiBody mb,
        MultiBodyConfig mbc,
        eigen.Vector3d com,
...
def computeCentroidalZMP(
        MultiBodyConfig mbc, eigen.Vector3d com, eigen.Vector3d comA, double altitude
):
    return eigen.Vector3dFromC(
        c_rbdyn.computeCentroidalZMP(deref(mbc.impl), com.impl, comA.impl, altitude)
    )


def IMPhi(sva.MotionVecd mv):
    return eigen.MatrixXdFromC(
        <c_eigen.MatrixXd>c_rbdyn.IMPhi(deref(mv.impl))
    )


def inertiaToVector(sva.RBInertiad rbi):
    return eigen.VectorXdFromC(
        <c_eigen.VectorXd>c_rbdyn.inertiaToVector(deref(rbi.impl))
    )
